...
function validateProductDisplayInfo(data) {
  ...
  if (!result)
    return {
      result: result,
      message: INVALID_PRODUCT_BID_PRICE,
    };
  ...
  if (!result)
    return {
      result: result,
      message: INVALID_PRODUCT_BIN_PRICE,
    };

  result = `${data.productBidMultiplication}`.match(
    NO_EMPTY_STRING
  );
  if (!result)
    return {
      result: result,
      message: INVALID_PRODUCT_BID_MULTIPLICATION,
    };

  result = `${data.productBidMultiplicationPeriod}`.match(
    NO_EMPTY_3_CHAR_REGEX
  );
  if (!result)
    return {
      result: result,
      message: INVALID_PRODUCT_BID_MULTIPLICATION_PERIOD,
    };

  result = `${data.productBidPeriod}`.match(
    NO_EMPTY_STRING
  );
  if (!result)
    return {
      result: result,
      message: INVALID_PRODUCT_BID_PERIOD,
    };
